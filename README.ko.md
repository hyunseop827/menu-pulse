...
2026년 7월 28일에 Menu Pulse `1.1.1`을 측정한 결과입니다.

- MacBook Air, Apple M1 8코어
- 메모리 16GB, SSD 256GB
- macOS 26.5.2
- 앱 실행 후 30초 예열
- 60초 동안 3초 간격으로 20회 측정

| 활성화한 지표 | CPU 평균 | CPU 최대 | RSS 평균 | RSS 최대 | Private dirty |
| --- | ---: | ---: | ---: | ---: | ---: |
| CPU + RAM (기본값) | 0.060% | 1.200% | 39.6MB | 39.7MB | 9.9MB |
...
예를 들어 기본 설정의 RSS는 39.6MB지만, 앱 전용으로 변경된 메모리에 가까운 Private dirty는 9.9MB였습니다.
CPU 최대값은 지표를 갱신하는 순간의 짧은 상승입니다.
결과는 실행 중인 다른 프로세스와 macOS 캐시 상태에 따라 달라질 수 있습니다.

같은 조건으로 측정하려면 앱을 실행하고 30초 기다린 뒤 다음을 실행하세요.
