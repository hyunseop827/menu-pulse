...
기본은 `CPU`와 `RAM`만 보여주고, 원하면 `HOT` (가장 뜨거운 센서)와 `DISK` (저장공간)을 추가할 수 있습니다.
...
그중 가장 뜨거운 유효 센서 값을 `HOT`으로 표시합니다.
기기 조합에 따라 항상 보장되지는 않으며, 읽지 못하면 `HOT:--°C`처럼 표시됩니다.
